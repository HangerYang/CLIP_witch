...
models = ['100K_truck_single', '100K_cat_single']
targets = ['selected_subset_truck_1000_target_ids', 'selected_subset_cat_1000_target_ids']
model_args = ['--model_name %s' % model for model in models]

for i in range(len(model_args)):
    _ = os.system('python verify_text_with_targets.py --device cuda:0 --run_name target \
        --eval_test_data_dir ../poisoning-gradient-matching/downloaded_data/cifar-10-batches-py \
        --targets_path ../poisoning-gradient-matching/poisons/targets_num_1_restart_4_attack_350/%s.log \
        --model_dir ../../../hyang/clip_witcher/CLIP/logs \
        --end_epoch 32 ' % targets[i]
         + model_args[i])

for args in model_args:
    _ = os.system('python verify_text_with_csv.py --device 0 --run_name all \
